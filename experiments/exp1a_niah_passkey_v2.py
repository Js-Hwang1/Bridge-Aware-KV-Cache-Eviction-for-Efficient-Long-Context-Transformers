"""
Experiment 1A: Needle-in-a-Haystack (Passkey Retrieval) - Version 2

FIXED: Now computes FRC on REAL attention scores (dense attention graph)
This is apple-to-apple comparison with H2O.

Both methods:
1. Start with dense attention computation
2. H2O: Select by magnitude
3. CAB: Select by curvature (bridges in the attention graph)
"""
...
# ============================================================================
# IMPROVED: Realistic Attention Model
# ============================================================================

class ImprovedRetrievalModel:
    """
    Improved model that creates STRUCTURED embeddings.

    Key changes:
    1. Filler tokens clustered together (high redundancy)
    2. Needle tokens distinct (low redundancy → bridge)
    3. Computes REAL attention scores to feed to FRC
    """

    def __init__(
        self,
        embed_dim: int = 128,
        num_heads: int = 1,  # Single head for simplicity
        device: str = 'cuda'
    ):
    ...
    def embed(self, tokens: List[str], needle_tokens: List[str]) -> torch.Tensor:
        ...
        Strategy:
        - Filler tokens: Cluster around filler_prototype (high similarity → redundant)
        - Needle tokens: Based on needle_prototype (distinct → bridge)
        """
        embeddings = []
        ...
        for token in tokens:
            if token in needle_set:
                # Needle token: distinct embedding
                noise = torch.randn(self.embed_dim, device=self.device) * 0.1
                emb = self.needle_prototype + noise
            else:
                # Filler token: clustered embedding
                noise = torch.randn(self.embed_dim, device=self.device) * 0.3
                emb = self.filler_prototype + noise

            embeddings.append(emb / emb.norm())  # Normalize
    ...
    def compute_dense_attention(
        ...
        """
        Compute FULL dense attention scores.
        This is the "real" attention graph we'll analyze with FRC.
        ...
        Returns:
            attention: [B, H, N, N] - softmax-normalized attention
        """
    ...
    def retrieve(
        ...
        # Check if ANY needle token is attended from the last position (query)
        N = len(context_tokens)
        if attention_mask.shape[0] < N:
            return False

        # Last token (query) attention to context
        query_attention = attention_mask[-1, :]

        # Success if any needle position is attended
...
# ============================================================================
# IMPROVED: Attention Methods (Apple-to-Apple)
# ============================================================================

def apply_h2o_mask_v2(
    attention_scores: torch.Tensor,
    sparsity: float,
    block_size: int = 64
) -> torch.Tensor:
    """
    H2O: Keep top-k blocks by MAGNITUDE from dense attention.

    Args:
        attention_scores: [B, H, N, N] - REAL dense attention scores
    ...
    # Blockify: Max pooling to block level
...
def apply_cab_mask_v2(
    ...
    """
    CAB: Keep blocks with LOWEST FRC (bridges) from dense attention graph.

    KEY CHANGE: FRC is computed on REAL attention scores, not random embeddings!
    ...
    # Blockify: Average pooling to block level
    M = (N + block_size - 1) // block_size
    block_scores = torch.zeros(M, M, device=scores_avg.device)
    ...
    # Compute FRC on the REAL attention graph (block-level)
    # Inputs: block_scores represents the attention affinity matrix

    # Node strengths (row sums)
    node_strength = block_scores.sum(dim=1)  # [M]

    # Triangle counts (2-hop paths)
    triangles = torch.matmul(block_scores, block_scores)  # [M, M]

    # FRC formula: Lower score = bridge (keep)
    # FRC = direct_connection - lambda * redundancy
    lambda_redundancy = 0.5
    frc_scores = block_scores - lambda_redundancy * triangles

    # Select blocks with LOWEST FRC (bridges)
    k_keep = max(1, int(M * M * (1 - sparsity)))
...
# ============================================================================
# Evaluation Harness (Updated for V2)
# ============================================================================
...
def evaluate_single_sample_v2(
    ...
    method: str,
    sparsity: float,
    model: ImprovedRetrievalModel,
    block_size: int = 64,
    device: str = 'cuda'
) -> bool:
    """
    Evaluate a single NIAH sample with IMPROVED model.

    KEY: Both methods see the same DENSE attention first.
    """
    # Tokenize
    ...
    # Expand to multi-head (just 1 head for simplicity)
    q = q.expand(1, model.num_heads, N, model.embed_dim)
    k = k.expand(1, model.num_heads, N, model.embed_dim)

    # STEP 1: Compute DENSE attention (both methods start here)
    dense_attention = model.compute_dense_attention(q, k)

    # STEP 2: Apply method-specific selection
    if method == 'full':
        mask = torch.ones(N, N, dtype=torch.bool, device=device)
    elif method == 'h2o':
        mask = apply_h2o_mask_v2(dense_attention, sparsity, block_size)
    elif method == 'cab':
        mask = apply_cab_mask_v2(dense_attention, sparsity, block_size)
    else:
...
def run_niah_experiment_v2(
    ...
    """Run full NIAH experiment with IMPROVED model."""
    model = ImprovedRetrievalModel(device=device)

    results = {method: {} for method in methods}

    print("=" * 80)
    print("EXPERIMENT 1A: NEEDLE-IN-A-HAYSTACK (VERSION 2 - FIXED)")
    print("=" * 80)
    print("IMPROVEMENTS:")
    print("  ✓ Structured embeddings (filler clustered, needle distinct)")
    print("  ✓ FRC computed on REAL dense attention graph")
    print("  ✓ Apple-to-apple comparison (both methods see same dense attention)")
    print("=" * 80)
    ...
    pbar = tqdm(total=total_configs, desc="Running NIAH v2")
            ...
            for sparsity in sparsity_levels:
                ...
                for method in methods:
                    ...
                    for sample in samples:
                        try:
                            success = evaluate_single_sample_v2(
...
def plot_niah_heatmap(
    ...
    needle_depths: List[float],
    sparsity: float,
    save_path: str = 'niah_heatmap_v2.png'
):
    """Generate heatmap."""
...
def main():
    ...
    num_samples = 5  # Increase for less noise

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(f"Device: {device}\n")

    # Run experiment
    results = run_niah_experiment_v2(
    ...
    with open('niah_results_v2.json', 'w') as f:
        json.dump(results_serializable, f, indent=2)
    print("\nSaved results: niah_results_v2.json")
    ...
    for sparsity in sparsity_levels:
        plot_niah_heatmap(
            ...
            needle_depths,
            sparsity,
            save_path=f'niah_heatmap_v2_sparsity_{int(sparsity*100)}.png'
        )

    # Print summary
    print("\n" + "=" * 80)
    print("RESULTS SUMMARY (VERSION 2)")
    print("=" * 80)
    ...
    print("  1. Structured embeddings (needle = distinct, filler = clustered)")
    print("  2. FRC computed on REAL dense attention graph")
    print("  3. Both methods start from same dense attention (fair comparison)")
    print("\nExpected: CAB >> H2O at high sparsity (preserves needle bridges)")
    print("=" * 80)
